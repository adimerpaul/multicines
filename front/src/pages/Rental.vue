<template>
  <q-page>
    <div class="row">
      ...
      <div class="col-12">
        <q-table label="FACTURA DE ALQUILER DE AMBIENTES" :rows="rentals" :columns="columns" :rows-per-page-options="[0]" dense>
          ...
          <template v-slot:body-cell-siatEnviado="props">
            <q-td :props="props" auto-width>
              <q-badge :color="props.row.siatEnviado==1?'green':'red'" :label="props.row.siatEnviado==1?'SI':'NO'" />
            </q-td>
          </template>
          <template v-slot:body-cell-siatAnulado="props">
            <q-td :props="props" auto-width>
              <q-badge :color="props.row.siatAnulado==1?'green':'red'" :label="props.row.siatAnulado==1?'SI':'NO'" />
            </q-td>
          </template>
        </q-table>
      </div>
...
export default {
  ...
  data(){
    return{
      dialogAnular:false,
      loading:false,
      fechaInicio:moment().add(-1,'month').startOf('month').format('YYYY-MM-DD'),
      ...
      columns:[
        ...
        {label:'numeroDocumento',name:'numeroDocumento',field:row=>row.client.numeroDocumento,sortable:true},
        {label:'client_id',name:'client_id',field:row=>row.client.nombreRazonSocial,sortable:true},
        {label:'cuf',name:'cuf',field:'cuf',sortable:true},
